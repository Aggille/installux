object frmCalculaSaldos: TfrmCalculaSaldos
  Left = 155
  Top = 137
  BorderStyle = bsToolWindow
  Caption = 'Rec'#225'lculo de saldo'
  ClientHeight = 77
  ClientWidth = 169
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnClose = FormClose
  PixelsPerInch = 96
  TextHeight = 13
  object BitBtn1: TBitBtn
    Left = 48
    Top = 16
    Width = 75
    Height = 25
    Caption = 'Iniciar'
    TabOrder = 0
    OnClick = BitBtn1Click
    Glyph.Data = {
      
      0F0F0F0F0F0F0F000000}
  end
  object gauge: TProgressBar
    Left = 8
    Top = 48
    Width = 150
    Height = 17
    TabOrder = 1
  end
  object IBTransaction1: TIBTransaction
    Active = True
    DefaultDatabase = frmPrincipal.idbInstalLux
    Left = 16
  end
  object qryEntrada: TIBQuery
    Database = frmPrincipal.idbInstalLux
    Transaction = IBTransaction1
    Active = True
    SQL.Strings = (
      'SELECT '
      'PRODUTO, SUM( ESTOQUE.QUANTIDADE )  AS ENTRADA FROM ESTOQUE'
      ''
      'WHERE TIPO = '#39'E'#39
      ''
      'GROUP BY PRODUTO'
      '')
    Left = 56
  end
  object dtsEntrada: TDataSource
    DataSet = qryEntrada
    Left = 56
    Top = 8
  end
  object dtsSaida: TDataSource
    DataSet = qrySaida
    Left = 88
    Top = 8
  end
  object qrySaida: TIBQuery
    Database = frmPrincipal.idbInstalLux
    Transaction = IBTransaction1
    Active = True
    SQL.Strings = (
      'SELECT '
      'PRODUTO, SUM( ESTOQUE.QUANTIDADE )  AS SAIDA FROM ESTOQUE'
      ''
      'WHERE TIPO  ='#39'S'#39
      ''
      'GROUP BY PRODUTO'
      '')
    Left = 88
  end
  object dtsProdutos: TDataSource
    DataSet = tblProdutos
    Left = 120
  end
  object tblProdutos: TIBDataSet
    Database = frmPrincipal.idbInstalLux
    Transaction = IBTransaction1
    OnCalcFields = tblProdutosCalcFields
    DeleteSQL.Strings = (
      'delete from PRODUTOS'
      'where'
      '  CODIGO = :OLD_CODIGO')
    InsertSQL.Strings = (
      'insert into PRODUTOS'
      '  (CODIGO, NOME, SALDO)'
      'values'
      '  (:CODIGO, :NOME, :SALDO)')
    RefreshSQL.Strings = (
      'Select '
      '  CODIGO,'
      '  NOME,'
      '  GRUPO,'
      '  UNIDADE,'
      '  PRECO,'
      '  ICMS,'
      '  IPI,'
      '  CUSTO,'
      '  ESTOQUEMINIMO,'
      '  TIPO,'
      '  SALDO,'
      '  PRATELEIRA'
      'from PRODUTOS '
      'where'
      '  CODIGO = :CODIGO')
    SelectSQL.Strings = (
      'SELECT'
      'CODIGO, NOME, SALDO'
      ''
      'FROM PRODUTOS ORDER BY CODIGO')
    ModifySQL.Strings = (
      'update PRODUTOS'
      'set'
      '  CODIGO = :CODIGO,'
      '  NOME = :NOME,'
      '  SALDO = :SALDO'
      'where'
      '  CODIGO = :OLD_CODIGO')
    Left = 120
    Top = 8
    object tblProdutosNOME: TIBStringField
      FieldName = 'NOME'
      Origin = '"PRODUTOS"."NOME"'
      Required = True
      Size = 50
    end
    object tblProdutosSALDO: TFloatField
      FieldName = 'SALDO'
      Origin = '"PRODUTOS"."SALDO"'
    end
    object tblProdutosENTRADA: TFloatField
      FieldKind = fkLookup
      FieldName = 'ENTRADA'
      LookupDataSet = qryEntrada
      LookupKeyFields = 'PRODUTO'
      LookupResultField = 'ENTRADA'
      KeyFields = 'CODIGO'
      Lookup = True
    end
    object tblProdutosSAIDA: TFloatField
      FieldKind = fkLookup
      FieldName = 'SAIDA'
      LookupDataSet = qrySaida
      LookupKeyFields = 'PRODUTO'
      LookupResultField = 'SAIDA'
      KeyFields = 'CODIGO'
      Lookup = True
    end
    object tblProdutosSALDO_NOVO: TFloatField
      FieldKind = fkCalculated
      FieldName = 'SALDO_NOVO'
      Calculated = True
    end
    object tblProdutosCODIGO: TIntegerField
      FieldName = 'CODIGO'
      Origin = 'PRODUTOS.CODIGO'
    end
  end
end
